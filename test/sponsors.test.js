...
describe("API Sponsor Routes", () => {
    ...
    it("should get all rubros", async () => {
        ...
        const response = await request(app).get("/api/rubros");
    ...
    it("should get a sponsor by ID", async () => {
        ...
        const response = await request(app).get(`/api/sponsors/${sponsor.idSponsor}`);
        expect(response.status).toBe(200);
    ...
    it("should update a sponsor by ID", async () => {
        ...
        const response = await request(app).put(`/api/sponsors/${sponsor.idSponsor}`).send(updatedSponsor);
        expect(response.status).toBe(200);
    ...
    it("should toggle the active state of a sponsor by ID", async () => {
        ...
        const response = await request(app).delete(`/api/sponsors/${sponsor.idSponsor}`);
        expect(response.status).toBe(200);
